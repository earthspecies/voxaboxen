...
for DATASET in Anuraset BV_slowed hawaii katydids_slowed MT powdermill OZF_slowed; do
    ...
    python main.py train-model --n-epochs=50 --batch-size=4 --encoder-type=beats --beats-checkpoint-fp=weights/BEATs_iter3_plus_AS2M_finetuned_on_AS2M_cpt2.pt --bidirectional --project-config-fp=projects/${DATASET}_experiment/project_config.yaml --name=${DATASET}-reproduction -t
done
