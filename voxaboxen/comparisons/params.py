...
def get_full_cfg(sound_event_args, detectron_args):
    ...
    cfg = get_cfg()
    # Defaults from ATST: win-len 64ms, hamming win, hop-len 10ms, 64 mel bins from 60 to 7800Hz, https://arxiv.org/pdf/2306.04186
    cfg.SPECTROGRAM = CN()
    cfg.SPECTROGRAM.N_FFT = int(64*sound_event_args.sr/1000) #400
    cfg.SPECTROGRAM.WIN_LENGTH = int(64*sound_event_args.sr/1000) #400
    cfg.SPECTROGRAM.HOP_LENGTH = int(10*sound_event_args.sr/1000) #200 #win_length // 2
    cfg.SPECTROGRAM.N_MELS = 64
    cfg.SPECTROGRAM.F_MIN = 60 # 20.
    cfg.SPECTROGRAM.F_MAX = 7800 # previously set to self.sr//2
    ...
    #dset_format_dir = cfg.DATASETS.TRAIN[0].removesuffix('train_info.csv')
    #cache_fp = os.path.join(dset_format_dir, 'comparison_dset_statistic_params.json')
    #if os.path.exists(cache_fp):
    #    with open(cache_fp) as f:
    #        cached_params = json.load(f)
    #    cfg.SPECTROGRAM.REF = cached_params['ref']
    #    cfg.MODEL.PIXEL_MEAN[0] = cached_params['pixel_mean']
    #    cfg.MODEL.PIXEL_STD[0] = cached_params['pixel_std']

    #else:
    collect_dataset_statistics(cfg, use_box_statistics=detectron_args.detectron_use_box_statistics);
        #to_cache = {'ref':cfg.SPECTROGRAM.REF, 'pixel_mean':cfg.MODEL.PIXEL_MEAN[0] , 'pixel_std':cfg.MODEL.PIXEL_STD[0]}
