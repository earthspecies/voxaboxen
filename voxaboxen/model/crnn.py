...
class CRNN(nn.Module):
    def __init__(self, args):
        ...
        self.spectrogram = torchaudio.transforms.MelSpectrogram(sample_rate = self.args.sr, n_fft=n_mels*6, hop_length = self.downsample_factor//2, n_mels=n_mels).to(device)
        ...
        self.head = nn.LSTM(hidden_size*(n_mels//8), rnn_hidden_size, num_layers=1, batch_first=True, bidirectional=True).to(device)
